...
# CSS for styling, including centering the image and full-width button
st.markdown(f"""
    {hide_sidebar_js}
    <style>
        ...
        /* Full-width recognition button */
        .stButton > button {{
            width: 100%;
            background-color: #1a237e; /* Deep blue color to match theme */
        ...
        body, .stApp {{
            background-color: #0d1b4b; /* Dark blue background */
            color: #e5e5e5;
        ...
        /* Sidebar Styling */
        .css-18e3th9 {{
            background-color: #1a237e !important; /* Matching theme for sidebar */
...
# Display the GIF image centered on the main screen
...
def video_stream(emotion_model, body_model):
    ...
    while st.session_state['run']:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 102, 204), 2) # Blue box for face
        ...
        for (x, y, w, h) in bodies:
            ...
            if frame_count % 5 == 0:
                ...
                body_confidence = np.max(prediction) * 100
                body_posture = f"{body_labels[body_label]} ({body_confidence:.2f}%)"

            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 102, 204), 2) # Blue box for body
...
st.markdown('<hr class="divider">', unsafe_allow_html=True)  # Divider line for UI separation

# Start/Stop buttons with loading spinner
if not st.session_state['run']:
    if st.button("Start Recognition"):
        st.session_state['run'] = True
        st.markdown("<script>hideSidebar();</script>", unsafe_allow_html=True)  # Hide sidebar when clicked
